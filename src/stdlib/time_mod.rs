...
fn time_now(_a: Vec<DgmValue>) -> Result<DgmValue, DgmError> { Ok(DgmValue::Int(chrono::Utc::now().timestamp())) }
fn time_now_ms(_a: Vec<DgmValue>) -> Result<DgmValue, DgmError> { Ok(DgmValue::Int(chrono::Utc::now().timestamp_millis())) }
...
fn time_parse(a: Vec<DgmValue>) -> Result<DgmValue, DgmError> {
    ...
    match (a.get(0), a.get(1)) {
        (Some(DgmValue::Str(s)), Some(DgmValue::Str(fmt))) => {
            let dt = NaiveDateTime::parse_from_str(s, fmt).map_err(|e| DgmError::RuntimeError { msg: format!("time.parse: {}", e) })?;
            Ok(DgmValue::Int(dt.and_utc().timestamp()))
        }
        _ => Err(DgmError::RuntimeError { msg: "time.parse(str, fmt) required".into() }),
    }
}
