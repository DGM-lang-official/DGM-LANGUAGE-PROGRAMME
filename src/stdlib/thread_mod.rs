...
fn thread_sleep(a: Vec<DgmValue>) -> Result<DgmValue, DgmError> {
    match a.first() { Some(DgmValue::Int(ms)) => { std::thread::sleep(std::time::Duration::from_millis(*ms as u64)); Ok(DgmValue::Null) } _ => Err(DgmError::RuntimeError { msg: "thread.sleep(ms) required".into() }) }
}
